<?php
	...
	class usuarioControlador extends usuarioModelo{
		...
		public function eliminar_usuario_controlador(){
			...
			/* comprobando el usuario princial */
			if($id==1){
				$alerta=[
					"Alerta"=>"simple",
					"Titulo"=>"Ocurrió un error inesperado",
			...
			if($eliminar_usuario->rowCount()==1){
				$alerta=[
					...
					"Texto"=>"El usuario ha sido eliminado del sistema con exito",
					"Tipo"=>"success"
				];
			}else{
				$alerta=[
					"Alerta"=>"simple",
					"Titulo"=>"Ocurrió un error inesperado",
					"Texto"=>"No hemos podido eliminar el usuario, por favor intente de nuevo",
					"Tipo"=>"error"
				];
			}
			echo json_encode($alerta);
		...
		public function actualizar_usuario_controlador(){
			...
			if($privilegio<1 || $privilegio>3){
				$alerta=[
					"Alerta"=>"simple",
					"Titulo"=>"Ocurrió un error inesperado",
					"Texto"=>"El privilegio seleccinado no es valido",
					"Tipo"=>"error"
				];
				echo json_encode($alerta);
				exit();
			...
			if(usuarioModelo::actualizar_usuario_modelo($datos_usuario_up)){
				...
				$alerta=[
					"Alerta"=>"simple",
					"Titulo"=>"Ocurrió un error inesperado",
					"Texto"=>"No hemos podido actualizar los datos, por favor intente de nuevo",
					"Tipo"=>"error"
				];
			}
			echo json_encode($alerta);
