		...
		public function agregar_empresa_controlador(){
			...
			if($agregar_empresa->rowCount()==1){
				...
				$alerta=[
					"Alerta"=>"simple",
					"Titulo"=>"Ocurrió un error inesperado",
					"Texto"=>"No hemos podido registrar la empresa, por favor intenta de nuevo",
					"Tipo"=>"error"
				];
			}
			echo json_encode($alerta);
		...
		public function actualizar_empresa_controlador(){
			...
			if(empresaModelo::actualizar_empresa_modelo($datos_empresa_up)){
				...
				$alerta=[
					"Alerta"=>"simple",
					"Titulo"=>"Ocurrió un error inesperado",
					"Texto"=>"No hemos podido actualizar los datos de la empres, por favor intenta de nuevo",
					"Tipo"=>"error"
				];
			}
			echo json_encode($alerta);
