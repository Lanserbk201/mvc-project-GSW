		...
		public function agregar_item_controlador(){
			...
			if($estado!="Habilitado" && $estado!="Deshabilitado"){
				$alerta=[
					"Alerta"=>"simple",
					"Titulo"=>"Ocurrió un error inesperado",
					"Texto"=>"El estado de item no coincide con el formato solicitado",
					"Tipo"=>"error"
				];
				echo json_encode($alerta);
				exit();
			...
			if($agregar_item->rowCount()==1){
				...
				$alerta=[
					"Alerta"=>"simple",
					"Titulo"=>"Ocurrió un error inesperado",
					"Texto"=>"No hemos podido registrar el item, por favor intenta de nuevo",
					"Tipo"=>"error"
				];
			}
			echo json_encode($alerta);
		...
		public function eliminar_item_controlador(){
			...
			if($eliminar_item->rowCount()==1){
				...
				$alerta=[
					"Alerta"=>"simple",
					"Titulo"=>"Ocurrió un error inesperado",
					"Texto"=>"No hemos podido eliminar el item del sistema, por favor intenta de nuevo",
					"Tipo"=>"error"
				];
			}
			echo json_encode($alerta);
		...
		public function actualizar_item_controlador(){
			...
			if(itemModelo::actualizar_item_modelo($datos_item_up)){
				...
				$alerta=[
					"Alerta"=>"simple",
					"Titulo"=>"Ocurrió un error inesperado",
					"Texto"=>"No hemos podido actualizar los datos del item, por favor intenta de nuevo",
					"Tipo"=>"error"
				];
			}
			echo json_encode($alerta);		
